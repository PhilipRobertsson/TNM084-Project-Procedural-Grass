...
public struct SurfaceJob<N> : IJobFor where N : struct, INoise {
	...
	public void Execute (int i) {
		...
		v.v0.tangent = half4(half(normalizer.x), half(tangentY.x), half(0f), half(-1f));
		v.v1.tangent = half4(half(normalizer.y), half(tangentY.y), half(0f), half(-1f));
		v.v2.tangent = half4(half(normalizer.z), half(tangentY.z), half(0f), half(-1f));
		v.v3.tangent = half4(half(normalizer.w), half(tangentY.w), half(0f), half(-1f));
	...
	public static JobHandle ScheduleParallel (
		...
		vertices =
			meshData.GetVertexData<SingleStream.Stream0>().Reinterpret<Vertex4>(9 * 4),
		settings = settings,
