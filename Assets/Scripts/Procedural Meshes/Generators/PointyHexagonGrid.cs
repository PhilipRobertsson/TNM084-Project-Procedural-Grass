    ...
    public struct PointyHexagonGrid : IMeshGenerator{
        ...
        public void Execute<S>(int z, S streams) where S : struct, IMeshStreams {
            ...
            for (int x = 0; x < Resolution; x++, vi += 7, ti += 6){
                ...
                vertex.texCoord0 = half2(half(0.5f - h), half(0.25f));
                streams.SetVertex(vi + 2, vertex);

                vertex.position.z = zCoordinates.z;
                vertex.texCoord0.y = half(0.75f);
                streams.SetVertex(vi + 3, vertex);

                vertex.position.x = center.x;
                vertex.position.z = zCoordinates.w;
                vertex.texCoord0 = half2(half(0.5f), half(1f));
                streams.SetVertex(vi + 4, vertex);

                vertex.position.x = xCoordinates.y;
                vertex.position.z = zCoordinates.z;
                vertex.texCoord0 = half2(half(0.5f + h), half(0.75f));
                streams.SetVertex(vi + 5, vertex);

                vertex.position.z = zCoordinates.y;
                vertex.texCoord0.y = half(0.25f);
        ...
        public Bounds Bounds => new Bounds(Vector3.zero,
            new Vector3(
                (Resolution > 1 ? 0.5f + 0.25f / Resolution : 0.5f) * sqrt(3f), 0.0f,0.75f + 0.25f / Resolution)
        );
