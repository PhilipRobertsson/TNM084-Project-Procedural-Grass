...
namespace ProceduralMeshes.Generators {
	...
	public struct GeoOctasphere : IMeshGenerator {
		...
		public void ExecuteRegular<S> (int i, S streams) where S : struct, IMeshStreams {
			...
			vertex.tangent.xz = half2(GetTangentXZ(vertex.position));
			vertex.tangent.w = half(-1f);
			vertex.texCoord0.x = half(rhombus.id * 0.25f + 0.25f);
			vertex.texCoord0.y = half((float)u / (2 * Resolution));
			...
			for (int v = 1; v < Resolution; v++, vi++, ti += 2) {
				...
				vertex.tangent.xz = half2(GetTangentXZ(vertex.position));
				vertex.texCoord0 = half2(GetTexCoord(vertex.position));
				streams.SetVertex(vi, vertex);
		...
		public void ExecutePolesAndSeam<S> (S streams) where S : struct, IMeshStreams {
			var vertex = new Vertex();
			vertex.tangent = half4(half(sqrt(0.5f)), half(0f), half(sqrt(0.5f)), half(-1f));
			vertex.texCoord0.x = half(0.125f);

			for (int i = 0; i < 4; i++) {
				vertex.position = vertex.normal = down();
				vertex.texCoord0.y = half(0f);
				streams.SetVertex(i, vertex);
				vertex.position = vertex.normal = up();
				vertex.texCoord0.y = half(1f);
				streams.SetVertex(i + 4, vertex);
				vertex.tangent.xz = half2(half(-vertex.tangent.z), half(vertex.tangent.x));
				vertex.texCoord0.x += half(0.25f);
			}
			...
			for (int v = 1; v < 2 * Resolution; v++) {
				...
				vertex.normal = vertex.position;
				vertex.texCoord0.y = half((float)v / (2 * Resolution));
